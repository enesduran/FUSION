...
class DNOOptions:
    ...
    batch_smoothness_scale: float = field(default=0, metadata={"help": "penalty for the batch smoothness"})
    ...
    def __call__(self, num_steps: int = None, start_x=None):
        ...
        with tqdm(range(num_steps)) as prog:
            for i in prog:
                ...
                info["loss_sum"] = loss.detach().cpu().numpy()
                
                loss = loss.mean()
                info["loss_diff"] = np.zeros(batch_size) 
                ...
                # Calculate the norm for each element in the batch
                grad_norms = self.current_z.grad.norm(p=2, dim=self.dims, keepdim=True)

                # Create a mask for non-zero norms to avoid division by very small values,
                # and normalize gradients only where the norm is above epsilon
                self.current_z.grad.data = torch.where(
                    grad_norms > self.eps,
                    self.current_z.grad.data / (grad_norms + self.eps),
                    torch.zeros_like(self.current_z.grad.data)
                )
                ...
                # grad mode
                self.current_z.grad.data /=  \
                        (self.current_z.grad.norm(p=2, dim=self.dims, keepdim=True) + self.eps)

                if torch.isnan(self.current_z.grad).any().item():
            ...
            for i in range(batch_size):
                ...
                for k in self.hist[0].keys(): 
                    ...
                    try:
                        ...
                        print(k, [info[k] for info in self.hist])
