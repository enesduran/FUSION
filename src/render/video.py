...
def add_text_moviepy(video, text, position='bottom', font_size=25, color='white', align='center'):
    import moviepy as mp
    ...
    video_text = TextClip(
        text=text,
        font="/is/cluster/eduran2/fonts/times.ttf", 
        color=color,
        method='label',
        text_align=align,
        size=(video.w, None),
    ...
    if 'bottom' in position:
        ...
        bg_height = video_text.h + 15
        bg_width = video.w
        background = ColorClip(size=(bg_width, bg_height),
                               color=(0, 0, 0)).with_opacity(0.6)
        
        # Center text on background
        video_text = video_text.with_position('center')
        text_with_bg = CompositeVideoClip([background, video_text])
        
    else:
        # For other positions, create smaller background
        bg_height = video_text.h + 30
        bg_width = video.w // 2
    ...
    # Set position of the text+background composite
    if isinstance(position, str):
        text_with_bg = text_with_bg.with_position(position)
    ...
    text_with_bg = text_with_bg.with_duration(video.duration)
...
class Video:
    ...
    def save(self, out_path) -> str:
        ...
        self.video.subclipped(0, self.duration).write_videofile(out_path, **self._conf)
...
def put_text(text: str, fname: str, outf: str, v=False):
    ...
    if fname == outf:
        temp = outf.replace('.mp4', '_.mp4')

    
    cmd_m.extend(['-i',fname, '-y', '-vf', f"drawtext=text='{text}':x=(w-text_w)/2:y=h-th-10:fontsize=28::box=1:boxcolor=black@0.6:boxborderw=5:fontcolor=white",
                    '-loglevel', 'quiet', '-c:a', 'copy', f'{temp}'])
