...
def get_frameidx(*, mode, nframes, exact_frame, frames_to_keep, superset_frames,
                 lengths=None, return_lists=True):

    if mode == "sequence":
        
        if superset_frames is not None:

            assert len(superset_frames) > frames_to_keep
            frameidx = np.linspace(0, len(superset_frames)-1, frames_to_keep)
            frameidx = np.round(frameidx).astype(int)

            frameidx = superset_frames[frameidx]
